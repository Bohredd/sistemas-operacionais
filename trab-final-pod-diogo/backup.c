...
void ordenarPorTempoRestante(Processo processos[], int numProcessos) {
    for (int i = 0; i < numProcessos - 1; i++) {
        for (int j = i + 1; j < numProcessos; j++) {
            if (processos[i].tempoRestante > processos[j].tempoRestante) {
...
void escalonadorSJFPreemptivo(Processo processos[], int numProcessos, int flagV) {
    ...
    while (processosPendentes != 0) {
        ...
        // printf("Processo executado ? %d Processos Pendentes ? %d \n", processoExecutado, processosPendentes);

        if (processoExecutado == 0) {
            // printf("NÃO FOI EXECUTADO NO CLOCK %d TEMPO \n", tempoAtual);
            if (flagV) {
        ...
        // so pra mostra que acabou dai no clock atual + 1
    ...
    while (processosPendentes > 0) {
        ...
        for (int fila = NUM_FILAS - 1; fila >= 0; fila--) {
            for (int i = 0; i < numFilas[fila]; i++) {
                ...
                printf("Prcesso p pid %d tempoRestante %d\n", p->pid, p->tempoRestante);
        ...
        // Debugging prints
        for (int i = 0; i < numProcessos; i++) {
            fprintf(stderr, "Debug - Processo %d: tempoRestante=%d, tempoReady=%d, tempoWait=%d, tempoSistema=%d\n",
                    processos[i].pid, processos[i].tempoRestante, processos[i].tempoReady, processos[i].tempoWait, processos[i].tempoSistema);
        }
    }
